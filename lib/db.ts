...
interface KisanMiterDB extends DBSchema {
  userProfile: {
    key: string
    value: {
      email: string
...
const DB_NAME = "KisanMiterDB"
const DB_VERSION = 2

let dbInstance: IDBPDatabase<KisanMiterDB> | null = null
...
export async function getDB() {
  ...
  dbInstance = await openDB<KisanMiterDB>(DB_NAME, DB_VERSION, {
    upgrade(db) {
...
export async function saveUserProfile(profile: KisanMiterDB["userProfile"]["value"]) {
...
// Session operations
export async function saveSession(session: Omit<KisanMiterDB["session"]["value"], "id">) {
  const db = await getDB()
...
export async function updateSession(updates: Partial<KisanMiterDB["session"]["value"]>) {
  const db = await getDB()
...
export async function saveMachinery(machinery: KisanMiterDB["machinery"]["value"]) {
  const db = await getDB()
...
export async function saveRentalRequest(request: KisanMiterDB["rentalRequests"]["value"]) {
  const db = await getDB()
...
export async function updateRentalRequestStatus(
  id: string,
  status: KisanMiterDB["rentalRequests"]["value"]["status"],
  additionalData?: Partial<KisanMiterDB["rentalRequests"]["value"]>
) {
  const db = await getDB()
